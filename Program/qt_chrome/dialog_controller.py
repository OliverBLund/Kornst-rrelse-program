...
class FramelessDialogChromeController(QObject):
    ...
    def configure(
        ...
        *,
        ...
        self._resize_margin = max(2, int(resize_margin))
        self._resize_enabled = bool(enable_resize)
    ...
    def _update_resize_cursor(self, pos: QPoint):
        if not self._resize_enabled or self.dialog.isMaximized() or self.dialog.isFullScreen():
            self._reset_cursor()
            return
    ...
    def eventFilter(self, watched, event):
        ...
        if etype == QEvent.Type.MouseButtonPress and hasattr(event, "button"):
            ...
            if self._resize_enabled and not self.dialog.isMaximized() and not self.dialog.isFullScreen():
