...
class TestComparisonTabSelectionState(unittest.TestCase):
    ...
    def test_statistics_unit_switch_updates_boxplot_axis_label(self):
        ...
        # m/s is the program default; switching the dropdown updates labels,
        # headers and values to the chosen unit.
        ...
        scope_value_ms = self.widget._stats_scope_table.item(0, 3).text()
        method_value_ms = self.widget._stats_method_table.item(0, 6).text()

        target_index = self.widget._stats_unit_combo.findData(HydraulicConductivityUnit.M_PER_DAY)
        self.widget._stats_unit_combo.setCurrentIndex(target_index)
        ...
        self.assertNotEqual(scope_value_ms, self.widget._stats_scope_table.item(0, 3).text())
        self.assertNotEqual(method_value_ms, self.widget._stats_method_table.item(0, 6).text())
